...
;
;
const SuggestionCard = /*#__PURE__*/ __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$index$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["default"].memo(_c = ({ suggestion, isNew, onAccept, onDismiss, onFocusSuggestion, onSelectReplacement })=>{
...
_c2 = SuggestionContext;
var _c, _c1, _c2;
__turbopack_context__.k.register(_c, "SuggestionCard$React.memo");
__turbopack_context__.k.register(_c1, "SuggestionCard");
__turbopack_context__.k.register(_c2, "SuggestionContext");
if (typeof globalThis.$RefreshHelpers$ === 'object' && globalThis.$RefreshHelpers !== null) {
    __turbopack_context__.k.registerExports(__turbopack_context__.m, globalThis.$RefreshHelpers$);
}
}),
...
function findDashConsistencyIssues(paragraphs) {
    const issues = [];
    for (const p of paragraphs){
        ...
        while((match = rangeRegex.exec(p.text)) !== null){
            ...
            if ((isRange || isNumeric || isProperNounPair) && !overlapsProtected(p.text, match.index, match.index + original.length)) {
                issues.push(createStyleSuggestion(p, match.index, original.length, "en-dash-range", "Use an en dash (–) for ranges or related terms.", [
                    `${left}–${right}`
                ]));
            }
        }
        ...
        while((match = doubleHyphen.exec(p.text)) !== null){
            issues.push(createStyleSuggestion(p, match.index, match[0].length, "em-dash-substitution", "Use an em dash (—) instead of double hyphens.", [
                `${match[1]}—${match[2]}`
            ]));
        }
    }
...
function findQuotationIssues(paragraphs, variant) {
    const issues = [];
    for (const p of paragraphs){
        if (isSkippedParagraph(p.text)) continue;
        const doubleRegex = /"([^"\n]+)"/g;
        ...
        while((match = doubleRegex.exec(p.text)) !== null){
            ...
            issues.push(createStyleSuggestion(p, match.index, match[0].length, "quotation-style", variant === "us" ? "Use curly double quotes for primary quotes." : "Use curly double quotes for nested quotes.", [
                `“${match[1]}”`
            ]));
        }
        const singleRegex = /(?<!\w)'([^'\n]+)'(?!\w)/g;
        while((match = singleRegex.exec(p.text)) !== null){
            if (overlapsProtected(p.text, match.index, match.index + match[0].length)) continue;
            issues.push(createStyleSuggestion(p, match.index, match[0].length, "quotation-style", variant === "uk" ? "Use curly single quotes for primary quotes." : "Use curly single quotes for nested quotes.", [
                `‘${match[1]}’`
            ]));
        }
    }
...
function findQuotePunctuationIssues(paragraphs, variant) {
    const issues = [];
    for (const p of paragraphs){
        if (isSkippedParagraph(p.text)) continue;
        if (variant === "us") {
            ...
            while((match = regex.exec(p.text)) !== null){
                if (overlapsProtected(p.text, match.index, match.index + match[0].length)) continue;
                issues.push(createStyleSuggestion(p, match.index, 2, "quote-punctuation-us", "Place periods and commas inside quotation marks (US).", [
                    `${match[1]}${match[0][0]}`
                ]));
            }
        } else {
            ...
            while((match = regex.exec(p.text)) !== null){
                if (overlapsProtected(p.text, match.index, match.index + match[0].length)) continue;
                issues.push(createStyleSuggestion(p, match.index, 2, "quote-punctuation-uk", "Place periods and commas outside quotation marks (UK).", [
                    `${match[0][1]}${match[1]}`
                ]));
            }
        }
...
function findApostropheIssues(paragraphs) {
    ...
    for (const p of paragraphs){
        ...
        while((match = regex.exec(p.text)) !== null){
            if (overlapsProtected(p.text, match.index, match.index + match[0].length)) continue;
            issues.push(createStyleSuggestion(p, match.index, match[0].length, "smart-apostrophe", "Use a curly typographic apostrophe.", [
                match[0].replace(/'/g, "’")
            ]));
        }
    }
...
function findHeadingIssues(paragraphs) {
    const issues = [];
    ...
    for (const p of paragraphs){
        ...
        if (isLikelyHeading(text)) {
            ...
            if (dominant === "title" && !isTitle) {
                issues.push(createStyleSuggestion(p, p.text.indexOf(text), text.length, "heading-casing", "Follow Title Case style.", [
                    toTitleCase(text)
                ], false));
            } else if (dominant === "sentence" && isTitle) {
                issues.push(createStyleSuggestion(p, p.text.indexOf(text), text.length, "heading-casing", "Follow Sentence case style.", [
                    toSentenceCase(text)
                ], false));
            }
        }
...
function findDateConsistencyIssues(paragraphs, dominant) {
    const issues = [];
    for (const p of paragraphs){
        if (isSkippedParagraph(p.text)) continue;
        ...
        while((match = wrongRegex.exec(p.text)) !== null){
            ...
            issues.push(createStyleSuggestion(p, match.index, match[0].length, "date-format", `Use dominant ${dominant.toUpperCase()} date style.`, [
                replacement
            ]));
        }
    }
...
function findAbbreviationIssues(paragraphs) {
    ...
    for (const p of paragraphs){
        ...
        while((match = defRegex.exec(p.text)) !== null){
            ...
            defined.add(abbr);
            if (!match[0].includes(" (")) {
                issues.push(createStyleSuggestion(p, match.index, match[0].length, "abbr-spacing", "Space before parentheses.", [
                    `${match[1]} (${match[2]})`
                ]));
            }
        }
...
function findSerialCommaIssues(paragraphs) {
    ...
    for (const p of paragraphs){
        ...
        while((match = regex.exec(p.text)) !== null){
            ...
            issues.push(createStyleSuggestion(p, match.index, match[0].length, "serial-comma", useSerial ? "Add serial comma." : "Remove serial comma.", [
                fix
            ]));
        }
    }
...
function findTableFigureNumberingIssues(paragraphs) {
    const issues = [];
    ...
    for (const p of paragraphs){
        ...
        if (tMatch && parseInt(tMatch[1]) !== tCount) {
            issues.push(createStyleSuggestion(p, p.text.toLowerCase().indexOf("table"), tMatch[0].length, "seq", `Expected Table ${tCount}.`, [
                `Table ${tCount}`
            ]));
        }
        if (tMatch) tCount++;
        ...
        if (fMatch && parseInt(fMatch[2]) !== fCount) {
            issues.push(createStyleSuggestion(p, p.text.toLowerCase().indexOf(fMatch[1].toLowerCase()), fMatch[0].length, "seq", `Expected ${fMatch[1]} ${fCount}.`, [
                `${fMatch[1]} ${fCount}`
            ]));
        }
        if (fMatch) fCount++;
...
function findOperatorSpacingIssues(paragraphs) {
    ...
    for (const p of paragraphs){
        ...
        while((match = regex.exec(p.text)) !== null){
            ...
            if (fix !== match[0]) {
                issues.push(createStyleSuggestion(p, match.index, match[0].length, "math-space", "Add space around operators.", [
                    fix
                ]));
            }
        }
...
// --- Helpers ---
function findRegexIssues(p, regex, type, message, replacements) {
    const issues = [];
    let match;
    while((match = regex.exec(p.text)) !== null){
...
function createStyleSuggestion(p, offset, length, ruleId, message, replacements, autoFixable = true) {
    return {
        id: Math.random().toString(36).substring(2, 11),
        paragraphIndex: p.paragraphIndex,
        paragraphText: p.text,
...
function getBlockAfterHeading(idx, ps) {
    const block = [];
...
function detectDateStyle(t) {
    const us = countMatches(t, monthDateRegex("g"));
    const uk = countMatches(t, dayMonthRegex("g"));
    return uk > us ? "uk" : "us";
}
function monthDateRegex(f = "") {
    return new RegExp(`\\b(${MONTHS.join("|")})\\s+(\\d{1,2}),\\s+(\\d{4})\\b`, f);
}
function dayMonthRegex(f = "") {
    return new RegExp(`\\b(\\d{1,2})\\s+(${MONTHS.join("|")})\\s+(\\d{4})\\b`, f);
}
...
function countMatches(t, r) {
    return Array.from(t.matchAll(r)).length;
}
if (typeof globalThis.$RefreshHelpers$ === 'object' && globalThis.$RefreshHelpers !== null) {
    __turbopack_context__.k.registerExports(__turbopack_context__.m, globalThis.$RefreshHelpers$);
}
...
"[project]/services/languagetool.ts [app-client] (ecmascript)", ((__turbopack_context__) => {
"use strict";
...
async function checkWithLanguageTool(paragraph, language) {
    try {
        ...
        const response = await fetch(__TURBOPACK__imported__module__$5b$project$5d2f$constants$2f$config$2e$ts__$5b$app$2d$client$5d$__$28$ecmascript$29$__["LANGUAGE_TOOL_ENDPOINT"], {
            method: "POST",
            headers: {
        ...
        if (!response.ok) {
            throw new __TURBOPACK__imported__module__$5b$project$5d2f$utils$2f$errors$2e$ts__$5b$app$2d$client$5d$__$28$ecmascript$29$__["AppError"](__TURBOPACK__imported__module__$5b$project$5d2f$utils$2f$errors$2e$ts__$5b$app$2d$client$5d$__$28$ecmascript$29$__["ErrorCode"].LANGUAGETOOL_API_ERROR, `LanguageTool API responded with status ${response.status}`);
        }
        ...
        if (error instanceof __TURBOPACK__imported__module__$5b$project$5d2f$utils$2f$errors$2e$ts__$5b$app$2d$client$5d$__$28$ecmascript$29$__["AppError"]) {
        ...
        try {
            (0, __TURBOPACK__imported__module__$5b$project$5d2f$utils$2f$errors$2e$ts__$5b$app$2d$client$5d$__$28$ecmascript$29$__["handleServiceError"])(error, __TURBOPACK__imported__module__$5b$project$5d2f$utils$2f$errors$2e$ts__$5b$app$2d$client$5d$__$28$ecmascript$29$__["ErrorCode"].LANGUAGETOOL_OFFLINE);
        } catch (e) {
...
"[project]/utils/concurrency.ts [app-client] (ecmascript)", ((__turbopack_context__) => {
"use strict";
...
"[project]/hooks/useScanner.ts [app-client] (ecmascript)", ((__turbopack_context__) => {
"use strict";
...
var __TURBOPACK__imported__module__$5b$project$5d2f$services$2f$languagetool$2e$ts__$5b$app$2d$client$5d$__$28$ecmascript$29$__ = __turbopack_context__.i("[project]/services/languagetool.ts [app-client] (ecmascript)");
var __TURBOPACK__imported__module__$5b$project$5d2f$utils$2f$checks$2e$ts__$5b$app$2d$client$5d$__$28$ecmascript$29$__ = __turbopack_context__.i("[project]/utils/checks.ts [app-client] (ecmascript)");
var __TURBOPACK__imported__module__$5b$project$5d2f$utils$2f$concurrency$2e$ts__$5b$app$2d$client$5d$__$28$ecmascript$29$__ = __turbopack_context__.i("[project]/utils/concurrency.ts [app-client] (ecmascript)");
var __TURBOPACK__imported__module__$5b$project$5d2f$constants$2f$config$2e$ts__$5b$app$2d$client$5d$__$28$ecmascript$29$__ = __turbopack_context__.i("[project]/constants/config.ts [app-client] (ecmascript)");
var __TURBOPACK__imported__module__$5b$project$5d2f$utils$2f$errors$2e$ts__$5b$app$2d$client$5d$__$28$ecmascript$29$__ = __turbopack_context__.i("[project]/utils/errors.ts [app-client] (ecmascript)");
...
function useScanner() {
    ...
    const scanningRef = (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$index$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["useRef"])(false);
    const shouldStopRef = (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$index$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["useRef"])(false);
    const suggestionsRef = (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$index$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["useRef"])([]);
    ...
    const [status, setStatus] = (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$index$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["useState"])("Open a Word document and scan.");
    const [latestId, setLatestId] = (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$index$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["useState"])(null);
    const [paragraphs, setParagraphs] = (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$index$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["useState"])([]);
    ...
    const setIfMounted = (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$index$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["useCallback"])({
        "useScanner.useCallback[setIfMounted]": (setter, value)=>{
            if (mountedRef.current) {
                setter(value);
            }
        }
    }["useScanner.useCallback[setIfMounted]"], []);
    const safeSetStatus = (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$index$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["useCallback"])({
        "useScanner.useCallback[safeSetStatus]": (value)=>{
            setIfMounted(setStatus, value);
        }
    }["useScanner.useCallback[safeSetStatus]"], [
        setIfMounted
    ]);
    const safeSetSuggestions = (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$index$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["useCallback"])({
        "useScanner.useCallback[safeSetSuggestions]": (value)=>{
            setIfMounted(setSuggestions, value);
        }
    }["useScanner.useCallback[safeSetSuggestions]"], [
        setIfMounted
    ]);
    const startScan = (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$index$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["useCallback"])({
        "useScanner.useCallback[startScan]": async (onlineEnabled, language)=>{
            ...
            try {
                // Clear old state
                await (0, __TURBOPACK__imported__module__$5b$project$5d2f$services$2f$word$2e$ts__$5b$app$2d$client$5d$__$28$ecmascript$29$__["setSuggestionHighlights"])(suggestionsRef.current, null);
                safeSetSuggestions([]);
                setIfMounted(setLatestId, null);
                const paragraphs = await (0, __TURBOPACK__imported__module__$5b$project$5d2f$services$2f$word$2e$ts__$5b$app$2d$client$5d$__$28$ecmascript$29$__["readParagraphs"])();
                setIfMounted(setParagraphs, paragraphs);
                const usable = paragraphs.filter({
                    "useScanner.useCallback[startScan].usable": (p)=>p.text.trim().length > 0
                }["useScanner.useCallback[startScan].usable"]);
                if (!usable.length) {
                ...
                const allFound = [];
                const styleSuggestions = (0, __TURBOPACK__imported__module__$5b$project$5d2f$utils$2f$checks$2e$ts__$5b$app$2d$client$5d$__$28$ecmascript$29$__["runDocumentStyleChecks"])(paragraphs);
                const concurrency = onlineEnabled ? __TURBOPACK__imported__module__$5b$project$5d2f$constants$2f$config$2e$ts__$5b$app$2d$client$5d$__$28$ecmascript$29$__["LANGUAGE_TOOL_CONCURRENCY"] : __TURBOPACK__imported__module__$5b$project$5d2f$constants$2f$config$2e$ts__$5b$app$2d$client$5d$__$28$ecmascript$29$__["LOCAL_SCAN_CONCURRENCY"];
                // Process in batches to update UI and apply highlights progressively
                for(let i = 0; i < usable.length; i += __TURBOPACK__imported__module__$5b$project$5d2f$constants$2f$config$2e$ts__$5b$app$2d$client$5d$__$28$ecmascript$29$__["SCAN_BATCH_SIZE"]){
                    if (shouldStopRef.current) {
                        safeSetStatus("Scan stopped by user.");
                        break;
                    }
                    const batch = usable.slice(i, i + __TURBOPACK__imported__module__$5b$project$5d2f$constants$2f$config$2e$ts__$5b$app$2d$client$5d$__$28$ecmascript$29$__["SCAN_BATCH_SIZE"]);
                    const batchParagraphIndexes = new Set(batch.map({
                        "useScanner.useCallback[startScan]": (para)=>para.paragraphIndex
                    }["useScanner.useCallback[startScan]"]));
                    const batchStyleSuggestions = styleSuggestions.filter({
                        "useScanner.useCallback[startScan].batchStyleSuggestions": (s)=>batchParagraphIndexes.has(s.paragraphIndex)
                    }["useScanner.useCallback[startScan].batchStyleSuggestions"]);
                    const batchResults = await (0, __TURBOPACK__imported__module__$5b$project$5d2f$utils$2f$concurrency$2e$ts__$5b$app$2d$client$5d$__$28$ecmascript$29$__["mapWithConcurrency"])(batch, concurrency, {
                        "useScanner.useCallback[startScan]": async (para)=>{
                            const local = (0, __TURBOPACK__imported__module__$5b$project$5d2f$utils$2f$checks$2e$ts__$5b$app$2d$client$5d$__$28$ecmascript$29$__["runLocalChecks"])(para);
                            const remote = onlineEnabled ? await (0, __TURBOPACK__imported__module__$5b$project$5d2f$services$2f$languagetool$2e$ts__$5b$app$2d$client$5d$__$28$ecmascript$29$__["checkWithLanguageTool"])(para, language) : [];
                            const merged = (0, __TURBOPACK__imported__module__$5b$project$5d2f$utils$2f$checks$2e$ts__$5b$app$2d$client$5d$__$28$ecmascript$29$__["mergeSuggestions"])(remote, local);
                            setIfMounted(setProgress, {
                                "useScanner.useCallback[startScan]": (prev)=>prev ? {
                                        ...prev,
                    ...
                    if (flatResults.length > 0) {
                        ...
                        const sorted = [
                            ...
                            "useScanner.useCallback[startScan].sorted": (a, b)=>{
                                if (a.paragraphIndex !== b.paragraphIndex) return a.paragraphIndex - b.paragraphIndex;
                                return a.offset - b.offset;
                            }
                        }["useScanner.useCallback[startScan].sorted"]);
                        safeSetSuggestions(sorted);
                        setIfMounted(setLatestId, flatResults[flatResults.length - 1].id);
                        // Apply highlights for this batch
                        await (0, __TURBOPACK__imported__module__$5b$project$5d2f$services$2f$word$2e$ts__$5b$app$2d$client$5d$__$28$ecmascript$29$__["setSuggestionHighlights"])(flatResults, "Yellow");
                    }
                    await (0, __TURBOPACK__imported__module__$5b$project$5d2f$utils$2f$concurrency$2e$ts__$5b$app$2d$client$5d$__$28$ecmascript$29$__["yieldToBrowser"])();
                }
                ...
                safeSetStatus((0, __TURBOPACK__imported__module__$5b$project$5d2f$utils$2f$errors$2e$ts__$5b$app$2d$client$5d$__$28$ecmascript$29$__["getFriendlyErrorMessage"])(error));
            } finally{
    ...
    const stopScan = (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$index$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["useCallback"])({
        "useScanner.useCallback[stopScan]": ()=>{
...
_s(useScanner, "ocJFQHxraBu/5DIDGO1w8l0kHHo=");
if (typeof globalThis.$RefreshHelpers$ === 'object' && globalThis.$RefreshHelpers !== null) {
    __turbopack_context__.k.registerExports(__turbopack_context__.m, globalThis.$RefreshHelpers$);
}
}),
...
function WritingAssistant() {
    _s();
    const { suggestions, setSuggestions, scanning, progress, status, setStatus, latestId, paragraphs, startScan, stopScan } = (0, __TURBOPACK__imported__module__$5b$project$5d2f$hooks$2f$useScanner$2e$ts__$5b$app$2d$client$5d$__$28$ecmascript$29$__["useScanner"])();
    ...
    const handleAccept = (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$index$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["useCallback"])({
        "WritingAssistant.useCallback[handleAccept]": async (id, replacement)=>{
            ...
            try {
                ...
                setStatus((0, __TURBOPACK__imported__module__$5b$project$5d2f$utils$2f$errors$2e$ts__$5b$app$2d$client$5d$__$28$ecmascript$29$__["getFriendlyErrorMessage"])(err));
            }
        }
    }["WritingAssistant.useCallback[handleAccept]"], [
        setSuggestions,
...
_s(WritingAssistant, "ACeWxoYaVzl4mp7c9O0NTmTjgic=", false, function() {
    return [
...
//# sourceMappingURL=_0w1tsri._.js.map